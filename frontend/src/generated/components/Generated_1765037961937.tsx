...
// Reusable Input Component with Red Border
  ...
  <div className="flex flex-col gap-2">
    ...
    <div className="relative">
      <input 
        ...
        className="w-full bg-[#121217] border border-red-600 rounded-sm px-3 py-2.5 text-sm text-gray-200 focus:outline-none focus:ring-1 focus:ring-red-500 placeholder-gray-600"
      />
...
// Reusable Select Component with Red Border
const SelectGroup = ({ label, value, placeholder }: { label: string, value?: string, placeholder?: string }) => (
  <div className="flex flex-col gap-2">
    <label className="text-[10px] text-gray-500 uppercase tracking-wide font-medium">{label}</label>
    <div className="relative">
      <select 
        className="w-full bg-[#121217] border border-red-600 rounded-sm px-3 py-2.5 text-sm text-gray-200 appearance-none focus:outline-none focus:ring-1 focus:ring-red-500"
        defaultValue={value || ""}
      >
...
const SystemCard = ({ id, version, action, isAdd }: { id: string, version: string, action: string, isAdd?: boolean }) => (
  <div className="bg-[#121217] border border-red-600 p-4 rounded-sm flex flex-col gap-4 min-h-[130px]">
    ...
    <button 
      className={`mt-auto w-full text-[10px] font-bold py-2 px-2 rounded-sm uppercase tracking-wider transition-colors border border-red-600
        ${isAdd 
          ? 'bg-blue-600 hover:bg-blue-700 text-white border-blue-600 hover:border-blue-700' 
          : 'bg-transparent hover:bg-red-900/20 text-gray-300'}`}
    >
      {action}
    </button>
  </div>
...
export default function ProjectScopeUI() {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white font-sans selection:bg-red-500/30">
      {/* Top Header */}
      <header className="flex items-center justify-between py-4 px-8 border-b border-red-600 bg-[#09090b]">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-red-600 rounded-full flex items-center justify-center font-bold text-xs shadow-lg shadow-red-900/20">jivs</div>
        </div>
      ...
      <main className="p-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-[#1e293b] border border-red-600 flex flex-col items-center justify-center rounded shadow-sm">
          ...
          <div className="flex gap-3">
            <button className="flex items-center gap-2 px-4 py-2 text-[10px] font-bold tracking-wider border border-red-600 rounded-sm text-gray-400 hover:bg-white/5 hover:text-white transition-colors">
        ...
        {/* Navigation Steps */}
        <div className="grid grid-cols-6 border border-red-600 rounded-sm overflow-hidden mb-12 bg-[#0f1014]">
          {steps.map((step, idx) => (
            <div 
              key={step.name} 
              className={`
                flex items-center justify-center py-4 px-2 text-[10px] font-bold tracking-wider cursor-pointer border-r border-red-600 last:border-r-0 transition-colors
        ...
        <div className="grid grid-cols-12 gap-10">
          ...
          <div className="col-span-7 flex flex-col gap-10">
            ...
            <div>
              <div className="flex justify-between items-center mb-5">
                ...
                <button className="flex items-center gap-2 px-3 py-1.5 text-[10px] font-bold tracking-wider border border-red-600 rounded-sm text-gray-500 hover:text-white hover:bg-white/5">
                  <LayoutGrid className="w-3 h-3" /> VIEW SCOPE
                </button>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="border border-dashed border-red-600 rounded-sm bg-[#0f1014] flex items-center justify-center min-h-[130px] cursor-pointer hover:bg-[#18181b] transition-colors group">
          ...
          <div className="col-span-5">
            ...
            <div className="bg-[#0f1014] border border-red-600 p-8 rounded-sm h-fit">
              ...
              <div className="flex justify-end pt-4">
                <button className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold py-3 px-8 rounded-sm flex items-center gap-2 transition-colors border border-red-600 shadow-lg shadow-blue-900/20">
